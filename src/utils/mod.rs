...
use dpi::{PhysicalSize, Position};
use serde::{Deserialize, Serialize};
use tao::{
    monitor::MonitorHandle,
    window::{Theme as TaoTheme, Window, WindowId as TaoWindowId},
};
use url::Url;
...
/// WebView that opened a new window.
#[derive(Debug)]
...
impl WindowWebViewMetaData {
    pub fn new(
        ...
        if !is_label_valid(&webview_label) || !is_label_valid(&window_label) {
...
pub fn find_monitor_for_position(
    ...
    monitors.into_iter().find(|monitor| {
        ...
        let window_position = window_position.to_physical::<i32>(monitor.scale_factor());
...
pub fn parse_proxy_url(url: &Url) -> crate::prelude::Result<ProxyConfig> {
    let host = url.host().map(|host| host.to_string()).unwrap_or_default();
    let port = url.port().map(|port| port.to_string()).unwrap_or_default();

    match url.scheme() {
        "http" => Ok(ProxyConfig::Http(ProxyEndpoint { host, port })),
        "socks5" => Ok(ProxyConfig::Socks5(ProxyEndpoint { host, port })),
        _ => Err(crate::prelude::Error::InvalidProxyUrl),
    }
}
